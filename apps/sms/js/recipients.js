(function(exports) {
  ...
  Recipients.View.prototype.render = function(opts) {
    ...
    // console.log('Rendering List: %d items', list.length);
  ...
  Recipients.View.prototype.handleEvent = function(proof, event) {
    ...
    switch (event.type) {
      ...
      case 'pan':
        // console.log( 'pan case', event, event.detail.absolute.dy );

        if (owner.length > 0) {
          Recipients.View.events.pan.call(this, event.detail.absolute);
      ...
      case 'keypress':
        // console.log( 'keypress case', event );
  ...
  Recipients.View.events = {
    pan: function(absolute) {
      var view = priv.get(this);
      var isTransitioning = Recipients.View.events.pan.isTransitioning;
      var value;
      ...
      Recipients.View.events.pan.isTransitioning = true;
      ...
      view.outer.addEventListener('transitionend', function te() {
        var last = view.inner.querySelector(':last-child');
        last.focus();
        last.scrollIntoView(true);
        Recipients.View.events.pan.isTransitioning = false;
        view.outer.removeEventListener('transitionend', te, true);
      });

      // Commence the transition
      view.outer.style.height = value;
      view.inner.style.height = value;
      view.inner.style.overflowY = 'scroll';
    }
  };
